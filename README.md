...
Visualizations typically consist of many graphical marks, such as [symbols](#symbols), [arcs](#arcs), [lines](#lines) and [areas](#areas). While some marks are easy enough to generate using [SVG path data](http://www.w3.org/TR/SVG/paths.html#PathData) or [Canvas path methods](http://www.w3.org/TR/2dcontext/#canvaspathmethods) directly, such as the rectangles of a bar chart, other shapes are complex, such as rounded annular sectors and centripetal Catmull–Rom splines. This module provides a variety of shape generators for your convenience.
