...
it("line(x, y) sets x and y", () => {
  ...
  assert.strictEqual(d3.line(x).x(), x);
  assert.strictEqual(d3.line(x, y).y(), y);
  assert.strictEqual(d3.line(3, 2).x()("aa"), 3);
  assert.strictEqual(d3.line(3, 2).y()("aa"), 2);
});

it("line.x(f)(data) passes d, i and data to the specified function f", () => {
  const data = ["a", "b"], actual = [];
  d3.line().x(function() { actual.push([].slice.call(arguments)); })(data);
  assert.deepStrictEqual(actual, [["a", 0, data], ["b", 1, data]]);
});

it("line.y(f)(data) passes d, i and data to the specified function f", () => {
  const data = ["a", "b"], actual = [];
  d3.line().y(function() { actual.push([].slice.call(arguments)); })(data);
  assert.deepStrictEqual(actual, [["a", 0, data], ["b", 1, data]]);
});

it("line.defined(f)(data) passes d, i and data to the specified function f", () => {
  const data = ["a", "b"], actual = [];
  d3.line().defined(function() { actual.push([].slice.call(arguments)); })(data);
  assert.deepStrictEqual(actual, [["a", 0, data], ["b", 1, data]]);
});

it("line.x(x)(data) observes the specified function", () => {
  const l = d3.line().x(function(d) { return d.x; });
...
it("line.y(y)(data) observes the specified function", () => {
  const l = d3.line().y(function(d) { return d.y; });
