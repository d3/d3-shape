...
it("line.curve(curveCardinalClosed)(data) generates the expected path", () => {
  const l = d3.line().curve(d3.curveCardinalClosed);
...
it("line.curve(curveCardinalClosed) uses a default tension of zero", () => {
  const l = d3.line().curve(d3.curveCardinalClosed.tension(0));
  assert.strictEqual(d3.line().curve(d3.curveCardinalClosed)([[0, 1], [1, 3], [2, 1], [3, 3]]), l([[0, 1], [1, 3], [2, 1], [3, 3]]));
});

it("line.curve(curveCardinalClosed.tension(tension)) uses the specified tension", () => {
  assertPathEqual(d3.line().curve(d3.curveCardinalClosed.tension(0.5))([[0, 1], [1, 3], [2, 1], [3, 3]]), "M1,3C1.166667,3,1.833333,1,2,1C2.166667,1,3.166667,3,3,3C2.833333,3,0.166667,1,0,1C-0.166667,1,0.833333,3,1,3");
});

it("line.curve(curveCardinalClosed.tension(tension)) coerces the specified tension to a number", () => {
  const l = d3.line().curve(d3.curveCardinalClosed.tension("0.5"));
  assert.strictEqual(d3.line().curve(d3.curveCardinalClosed.tension(0.5))([[0, 1], [1, 3], [2, 1], [3, 3]]), l([[0, 1], [1, 3], [2, 1], [3, 3]]));
});

it("area.curve(curveCardinalClosed)(data) generates the expected path", () => {
  const a = d3.area().curve(d3.curveCardinalClosed);
...
it("area.curve(curveCardinalClosed) uses a default tension of zero", () => {
  const a = d3.area().curve(d3.curveCardinalClosed.tension(0));
  assert.strictEqual(d3.area().curve(d3.curveCardinalClosed)([[0, 1], [1, 3], [2, 1], [3, 3]]), a([[0, 1], [1, 3], [2, 1], [3, 3]]));
});

it("area.curve(curveCardinalClosed.tension(tension)) uses the specified tension", () => {
  assertPathEqual(d3.area().curve(d3.curveCardinalClosed.tension(0.5))([[0, 1], [1, 3], [2, 1], [3, 3]]), "M1,3C1.166667,3,1.833333,1,2,1C2.166667,1,3.166667,3,3,3C2.833333,3,0.166667,1,0,1C-0.166667,1,0.833333,3,1,3M2,0C1.833333,0,1.166667,0,1,0C0.833333,0,-0.166667,0,0,0C0.166667,0,2.833333,0,3,0C3.166667,0,2.166667,0,2,0");
});

it("area.curve(curveCardinalClosed.tension(tension)) coerces the specified tension to a number", () => {
  const a = d3.area().curve(d3.curveCardinalClosed.tension("0.5"));
  assert.strictEqual(d3.area().curve(d3.curveCardinalClosed.tension(0.5))([[0, 1], [1, 3], [2, 1], [3, 3]]), a([[0, 1], [1, 3], [2, 1], [3, 3]]));
});
